...
async function proxyToBackend(request: NextRequest, userId: string): Promise<NextResponse> {
  ...
  // 前端 /api/contact-members/[userId] → 后端 /api/contact-members/[userId]
  ...
  try {
    ...
    if (!contentType.includes('application/json')) {
      ...
      console.error('[ContactMember Proxy] Backend returned non-JSON:', text.substring(0, 200));
    ...
    console.error('[ContactMember Proxy] Proxy request failed:', error);
